...
# Log output
STD_LIB_LOG="/tmp/StandardLibraryTest/log.txt"

# Use a unit test that requires mutex and cell
echo "Starting RMC codegen for the Rust standard library"
cd /tmp
if [ -d StandardLibraryTest ]; then rm -rf StandardLibraryTest; fi
cargo new StandardLibraryTest
cd StandardLibraryTest
